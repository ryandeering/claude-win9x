    ...
    TimeSpan? writeTimeout = null) : IFileSystemService
{
    ...
    public (string ZipPath, long Size)? CreateBundle(string sourcePath, string? outputName, string? allowedBasePath = null)
    {
        ...
        var baseDir = allowedBasePath != null
            ? Path.GetFullPath(allowedBasePath + Path.DirectorySeparatorChar)
            : Path.GetFullPath(Directory.GetCurrentDirectory() + Path.DirectorySeparatorChar);

        if (!fullSourcePath.StartsWith(baseDir, StringComparison.OrdinalIgnoreCase))
        {
            logger.LogWarning("Bundle rejected (path escape): {SourcePath} not within {BaseDir}", sourcePath, baseDir);
            return null;
        }

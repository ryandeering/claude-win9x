...
namespace ClaudeWin9xServer.Endpoints;
...
public static class EndpointMappings
{
    ...
    {
        app.MapPost("/fs/bundle", Results<Ok<BundleResponse>, BadRequest<ErrorResponse>, StatusCodeHttpResult> (BundleRequest request, IFileSystemService fileSystemService, ISessionService sessionService) =>
        {
            ...
            // Get the session's working directory to use as the allowed base path
            string? allowedBasePath = null;
            if (!string.IsNullOrEmpty(request.SessionId))
            {
                allowedBasePath = sessionService.GetWorkingDirectory(request.SessionId);
            }

            var result = fileSystemService.CreateBundle(request.SourcePath, request.OutputName, allowedBasePath);
